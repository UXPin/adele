...
2. `npm run template { name of the company }` - it will automatically generate an empty JSON in the catalog src/data/systems. The file is going to have a name that you mentioned in the CLI (for clarity sake let's keep it as the company name). If you haven't filled the name, look for a file 'new-system.JSON' and change its name to {company}.JSON.
